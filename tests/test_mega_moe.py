import os
# Enable paddle compat proxy before importing torch
from paddle.compat.proxy import enable_compat
enable_compat()
...
def test(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    def create_inputs():
        global x, x_bf16, topk_idx, topk_weights, l1_weights, l2_weights, transformed_l1_weights, transformed_l2_weights
    ...
    dist_print(f' > Buffer: {buffer.buffer.numel() / 2 ** 30:.3f} GiB', once_in_node=True)
    dist_print(once_in_node=True)
    ...
    if args.ncu_profile_only:
        ...
        paddle_dist.barrier()
        buffer.destroy()
        paddle_dist.destroy_process_group()
        return
    ...
    def run_baseline():
        ...
        deep_gemm.m_grouped_fp8_fp4_gemm_nt_contiguous(
            ...
            use_psum_layout=True, recipe=(1, 1, 32))
        combined_x, _, _ = ep_buffer.combine(l2_y, handle=handle)
        return combined_x, cumulative_local_expert_recv_stats_baseline
    ...
    if is_legacy_loaded and num_correctness_tests > 0:
        ...
        for i in range(num_correctness_tests):
            ...
            for idx, (fused_result, baseline_result) in enumerate(zip(fused_results, baseline_results)):
                is_equal = (fused_result == baseline_result).all().item()
                assert is_equal, \
                    f'Mismatch result#{idx}: max diff = {(fused_result.float() - baseline_result.float()).abs().max().item()}'
    ...
    # Benchmark
    t_fused = bench(run_fused, num_warmups=5, num_tests=30)
    t_baseline = bench(run_baseline, num_warmups=5, num_tests=30) if is_legacy_loaded else 0
...
def _spawn_processes(fn, args_tuple, nprocs):
    """Simple replacement for torch.multiprocessing.spawn using Python multiprocessing."""
    ctx = mp.get_context('spawn')
    processes = []
    for i in range(nprocs):
        p = ctx.Process(target=_worker_wrapper, args=(fn, i, *args_tuple))
        p.start()
        processes.append(p)
    ...
    for i, p in enumerate(processes):
        p.join()
        if p.exitcode != 0:
            raise RuntimeError(f'Process {i} exited with code {p.exitcode}')


def _worker_wrapper(fn, *args):
    """Wrapper that enables paddle compat proxy in each child process."""
    from paddle.compat.proxy import enable_compat
    enable_compat()
    fn(*args)
...
if __name__ == '__main__':
    ...
    if args.local_rank_idx is not None:
        ...
        _spawn_processes(test, (num_processes, args), num_processes)
