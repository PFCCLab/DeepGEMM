...
template <uint32_t BLOCK_M, uint32_t BLOCK_N,
          ...
          GemmType kGemmType, bool kWithAccumulation, bool kWithBias,
          typename cd_dtype_t,
...
sm100_store_cd_swap_ab(const utils::PatternVisitor<pattern_cd_t>& smem_cd, uint32_t& tma_stage_idx,
                       ...
                       const uint32_t& base_m_idx, const uint32_t& base_n_idx, const uint32_t& shape_n,
        ...
        for (uint32_t i = 0; i < STORE_BLOCK_M / kNumSwizzleAtomRows; ++ i) {
            ...
            if constexpr (cute::is_same_v<cd_dtype_t, float>) {
                ...
                if constexpr (kWithBias) {
                    ...
                    const auto raw_bias_n_idx = epilogue_type_t::template apply_index_n<STORE_BLOCK_N_ATOM>(
                        base_n_idx + (epilogue_warp_idx / kNumWarpsPerAtom) * STORE_BLOCK_N_ATOM) + lane_idx;
                    const auto bias_n_idx = raw_bias_n_idx < shape_n ? raw_bias_n_idx : shape_n - 1;
                    const auto bias = static_cast<float>(bias_ptr[bias_n_idx]);
                    #pragma unroll
                    for (uint32_t row = 0; row < kNumSwizzleAtomRows; ++ row) {
                        const float value = __uint_as_float(values[row]);
                        values[row] = __float_as_uint(value + bias);
                    }
                }
                ...
                if constexpr (kWithBias) {
                    // For SM100_TMEM_LOAD_16dp256b1x, the per-lane register layout is:
                    //   reg 0: TMEM dp = T/4,        col = 2*(T%4)
                    //   reg 1: TMEM dp = T/4,        col = 2*(T%4) + 1
                    //   reg 2: TMEM dp = T/4 + 8,    col = 2*(T%4)
                    //   reg 3: TMEM dp = T/4 + 8,    col = 2*(T%4) + 1
                    // The second TMEM_LOAD with bit 20 set shifts dp by +16, so values[4..7]
                    // share the same per-lane (col) layout but with dp += 16.
                    // For swap_ab: dp = N (output column), col = M (output row).
                    // Therefore values[2k] and values[2k+1] share N = T/4 + k*8 (k=0..3),
                    // and only differ in M. So we apply ONE bias per pair.
                    // Two warps within an atom split N: warp_inner=0 -> N in [0, 32),
                    // warp_inner=1 -> N in [32, 64) (atom-relative).
                    constexpr uint32_t kNumNPerWarp = 32;
                    const auto warp_n_base = epilogue_type_t::template apply_index_n<STORE_BLOCK_N_ATOM>(
                        base_n_idx + (epilogue_warp_idx / kNumWarpsPerAtom) * STORE_BLOCK_N_ATOM)
                        + (epilogue_warp_idx % kNumWarpsPerAtom) * kNumNPerWarp;
                    const auto lane_n_base = lane_idx / 4;
                    #pragma unroll
                    for (uint32_t k = 0; k < kNumSwizzleAtomRows / 2; ++ k) {
                        const auto raw_n = warp_n_base + lane_n_base + k * 8;
                        const auto bias_n_idx = raw_n < shape_n ? raw_n : shape_n - 1;
                        const auto bias_v = static_cast<float>(bias_ptr[bias_n_idx]);
                        const auto v0 = __uint_as_float(values[2 * k]);
                        const auto v1 = __uint_as_float(values[2 * k + 1]);
                        values[2 * k]     = __float_as_uint(v0 + bias_v);
                        values[2 * k + 1] = __float_as_uint(v1 + bias_v);
                    }
                }
