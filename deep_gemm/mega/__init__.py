...
class SymmBuffer:
    def __init__(self, group: dist.ProcessGroup,
        ...
        num_bytes, slice_input_buffers = _C.get_symm_buffer_size_for_mega_moe(
            group.world_size, num_experts,
            num_max_tokens_per_rank, num_topk,
            hidden, intermediate_hidden,
            use_fp8_dispatch, activation
        )
        import paddle
        self.buffer = symm_mem.empty(num_bytes, dtype=torch.int8)
        self.handle = symm_mem.rendezvous(self.buffer, group=group)
        # Access buffer_ptrs from the underlying C++ handle
        self.buffer_ptrs = self.handle._handle.buffer_ptrs
        self.buffer.zero_()
        paddle_dist.barrier(group=group)
...
def _transpose_sf_for_utccp(sf: torch.Tensor) -> torch.Tensor:
    ...
    # Do the 4x32 transpose in data while preserving MN-major strides
    result = (sf.reshape(num_groups, -1, 4, 32, packed_sf_k)
                .transpose(2, 3)
                .reshape(num_groups, mn, packed_sf_k))
    # Create MN-major output: allocate as [num_groups, packed_sf_k, mn] contiguous,
    # then transpose to get [num_groups, mn, packed_sf_k] with stride [mn*packed_sf_k, 1, mn]
    out = torch.empty(num_groups, packed_sf_k, mn, dtype=sf.dtype, device=sf.device)
    out_mn = out.transpose(1, 2)  # [num_groups, mn, packed_sf_k] with MN-major strides
    out_mn.copy_(result)
    return out_mn
