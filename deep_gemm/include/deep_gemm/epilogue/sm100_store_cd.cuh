    ...
    for (uint32_t w = 0; w < kNumMWaves; ++ w) {
        ...
        for (uint32_t s = 0; s < kNumStores; ++ s, advance_store_pipeline()) {
            ...
            for (uint32_t i = 0; i < STORE_BLOCK_N / kNumElemsPerBankGroup; ++ i) {
                ...
                auto add_bias = [&](uint32_t (&values)[kNumElemsPerBankGroup]) {
                    if constexpr (kWithBias) {
                        const auto raw_bias_n_idx = n_idx + i * kNumElemsPerBankGroup;
                        const auto max_bias_n_idx = shape_n > kNumElemsPerBankGroup ? shape_n - kNumElemsPerBankGroup : 0u;
                        const auto bias_n_idx = raw_bias_n_idx < max_bias_n_idx ? raw_bias_n_idx : max_bias_n_idx;
                        const auto current_bias_ptr = bias_ptr + bias_n_idx;
                        #pragma unroll
                        for (uint32_t o = 0; o < kNumElemsPerBankGroup; ++ o) {
                            const float value = __uint_as_float(values[o]);
                            values[o] = __float_as_uint(value + static_cast<float>(current_bias_ptr[o]));
                        }
                    }
                };
