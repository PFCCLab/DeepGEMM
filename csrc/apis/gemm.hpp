...
static bool early_return(const int& m, const int &n, const int& k,
    ...
    if (bias.has_value()) {
        const auto [n_] = get_shape<1>(bias.value());
        DG_HOST_ASSERT(n == n_);
        DG_HOST_ASSERT(bias.value().is_cuda());
        DG_HOST_ASSERT(bias.value().stride(0) == 1);
    ...
    if (k == 0) {
        ...
        if (bias.has_value()) {
            auto mutable_d = const_cast<torch::Tensor&>(d);
            mutable_d.copy_(paddle::experimental::add(d._PD_GetInner(), bias.value().view({1, n}).expand({m, n})._PD_GetInner()));
        }
        return true;
    }
...
static void fp8_fp4_gemm_nt(const std::pair<torch::Tensor, torch::Tensor>& a,
    ...
    if (early_return(m, n, k, d, c, bias))
        return;
    ...
    if (arch_major == 9 and sfa.scalar_type() == torch::kFloat) {
        ...
        sm100_fp8_fp4_gemm_1d1d(a.first, sfa, b.first, sfb, c, d, m, n, k, gran_k_a, gran_k_b,
                                major_a, major_b, compiled_dims, std::nullopt, bias);
    } else {
...
static void fp8_fp4_gemm_nn(const std::pair<torch::Tensor, torch::Tensor>& a,
    ...
    fp8_fp4_gemm_nt(a, {b.first.transpose(0, 1), b.second.transpose(0, 1)},
                    d, c, recipe, recipe_a, recipe_b, compiled_dims, disable_ue8m0_cast, bias);
}
